...
import { Search, Plus, FileSpreadsheet, ChevronDown, Map, Trash2, Edit3, Copy, Receipt, Pencil, Check, Loader2, X } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-[var(--card-border)] pb-3">
        <div>
          <h1 className="text-xl font-serif font-normal tracking-tight flex items-center gap-2">
            <Map className="w-5 h-5 text-[#D97757] opacity-80" />
            Tour Packages
          </h1>
          ...
          <table className="w-full text-left whitespace-nowrap">
            <thead className="sticky top-0 z-10 border-b border-[var(--card-border)] bg-[var(--sidebar-bg)] backdrop-blur-md text-[10px] uppercase tracking-wider opacity-90 shadow-xs">
              <tr>
                ...
                <th className="px-3 py-2 font-semibold">Tour Name</th>
              ...
              {paginatedItems.map(s => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-[var(--sidebar-bg)] transition-colors">
                    ...
                    <td className="px-3 py-1.5 text-xs">{details?.travel_date || '—'}</td>
                    ...
                    <td className="px-3 py-1.5 text-xs text-right font-mono">{discount.toLocaleString()}</td>
                    ...
                    <td className="px-3 py-1.5 text-xs">{details?.supplier_name || '—'}</td>
                    <td className="px-3 py-1.5 text-xs max-w-[150px] truncate" title={details?.tour_plans}>{details?.tour_plans || '—'}</td>
                    <td className="px-3 py-1.5 text-xs max-w-[150px] truncate" title={details?.remark}>{details?.remark || '—'}</td>
                    <td className="px-3 py-1.5 text-xs">
