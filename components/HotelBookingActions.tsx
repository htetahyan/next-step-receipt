...
export default function HotelBookingActions({ data }: { data: HotelBookingData }) {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const sanitizedName = (data.bookingId || 'hotel').replace(/[/\\?%*:|"<>]/g, '-');
      ...
      pdf.save(`hotel-booking-${sanitizedName}.pdf`);
    } catch (e: any) {
  ...
  return (
    <div className="space-y-6">
       <div className="flex items-center justify-between print:hidden">
        <h1 className="text-xl font-bold text-slate-800 dark:text-white">Hotel Booking: {data.bookingId}</h1>
